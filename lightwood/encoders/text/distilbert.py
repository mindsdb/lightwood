...
class DistilBertEncoder:
    ...
    def prepare_encoder(self, priming_data, training_data=None):
        ...
        if training_data['targets'][0]['output_type'] == COLUMN_DATA_TYPES.NUMERIC and CONFIG.TRAIN_TO_PREDICT_TARGET:
            self._model_type = 'classifier'
            self._model = self._classifier_model_class.from_pretrained(self._pretrained_model_name).to(self.device)
            ...
            self._head = DefaultNet(ds=None, dynamic_parameters={},shape=[128,64,1], selfaware=False)
...
if __name__ == "__main__":
    ...
    enc.prepare_encoder(priming_data, training_data={'targets': [{'output_type': COLUMN_DATA_TYPES.NUMERIC, 'encoded_output': primting_target}]})
    predicted_target = enc.encode(test_data)

    encoder_accuracy = accuracy_score(list(predicted_target), test_target)
