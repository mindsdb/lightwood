        ...
        for vector in encoded_values:
            if self.is_target:
                if np.isnan(vector[0]) or vector[0] == float('inf') or np.isnan(vector[1]) or vector[1] == float('inf') or np.isnan(vector[2]) or vector[2] == float('inf'):
                    ...
                    if decode_log:
                        real_value = [max(0, math.exp(vector[i*2 + 1])) for i in range(1 + self.extra_outputs)]
                    else:
                        real_value = [max(0, vector[2*i + 2] * self._abs_mean) for i in range(1 + self.extra_outputs)]
