...
class CategoricalAutoEncoder(BaseEncoder):
    ...
    def prepare(self, train_priming_data, dev_priming_data):
        ...
        log.info('Preparing a categorical autoencoder, this may take up to ' + str(self.stop_after) + " seconds.")

        train_loader, dev_loader = self._prepare_AE_input(train_priming_data, dev_priming_data)
    ...
    def _prepare_AE_input(self, train_priming_data: pd.Series, dev_priming_data: pd.Series) -> Tuple[DataLoader, DataLoader]:
        """
        ...
        if len(dev_priming_data) > 0:
            priming_data = pd.concat([train_priming_data, dev_priming_data]).astype(str).tolist()
        else:
    ...
    def _prepare_catae(self, train_loader: DataLoader, dev_loader: DataLoader):
        ...
        best_model, _, _ = gym.fit(train_loader,
                                   dev_loader,
